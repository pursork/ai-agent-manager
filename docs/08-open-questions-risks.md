...
| 17 | `aam session adopt --summarize`（`05.8`）需要调用一个 Provider 生成摘要，但 `Provider` trait 之前只有 `materialize`/`verify`/`api_key`，没有"给一段文本，返回补全"的通用能力 | `05.8`；**已实现** | `Provider` trait 加了 `complete(prompt) -> Result<String, CompleteError>`。**协议选择先核实后动手**：Codex 端 `codex_toml.rs` 已经写死 `wire_api = "responses"`（OpenAI Responses API），跟 Claude 端查证的 Anthropic 官方文档（`platform.claude.com/docs/en/api/messages`：`POST {base_url}/v1/messages`，认证头是 `X-Api-Key` 不是 `Authorization: Bearer`）是两套不同协议，不能混用；`complete()` 选择 Anthropic Messages API，因为 `materialize()` 早就确定 `CpaProvider`/`DeepSeekProvider` 要能服务 Claude Code 就必须在 `{base_url}/v1/messages` 支持这个协议——是复用既有假设，不是新猜测。摘要输入是会话原始文件片段，不做结构化解析（`05.8`）。排期已完成，只剩 `aam-skills` 的 Phase 3 子集（`09.6`/`09.7`）这一轮，做完进 Phase 4 |
