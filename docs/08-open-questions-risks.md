...
| 17 | `aam session adopt --summarize`（`05.8`）需要调用一个 Provider 生成摘要，但 `Provider` trait 今天只有 `materialize`/`verify`/`api_key`，没有"给一段文本，返回补全"的通用能力 | `05.8`；`aam session adopt` 里 `--summarize` 尚未实现（CLI 文档已注明） | **排在"补完 Phase 3"的下一轮**：给 `aam-switcher::Provider` 加一个 `complete(prompt) -> Result<String, _>` 之类的方法，Claude/CPA/DeepSeek 各自实现；不跟 #8 的手动关联/跨设备 resume 混在一轮，因为这是一个会影响 `Provider` trait 公开签名的改动，值得单独过一遍设计。再之后一轮是 `aam-skills` 的 Phase 3 子集（`09.6`/`09.7`：`aam skills scan`/完整版 `adopt`/`check-updates`/`update`），两轮都做完再进 Phase 4 |
