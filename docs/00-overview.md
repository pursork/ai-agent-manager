...
- G6：Skills（`~/.claude/skills`、`$HOME/.agents/skills`）在同一台机器上跨 Claude/Codex 共享、跨 Claude Profile 保持一致，并且能一键发现本机已存在但未纳管的 skills/会话、补齐台账——但"发现"和"对外同步"是两回事，扫描出来的东西默认不出本机，需要用户逐项显式批准（见 `05`、`09`）。
...
- **不做 Skills 的加密云同步/版本合并**——G6 里 Skills 内容本身走明文/git，不接入 `04` 的加密 WebDAV 通道；也不做真正的包管理器（依赖解析、语义化版本），GitHub 来源的更新检查只是"diff 并提示"，不是 npm/cargo 那种生态（见 `09`）。
...
- `09-skills-management.md` —— Skills 跨工具共享与发现/纳管（G6）
