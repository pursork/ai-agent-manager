...
对应文档：`06.3`-`06.4`。沿用 Phase 4 的分轮节奏，每轮独立验收：

- **Round 1（已完成，已自动化验证）**：`iced_term` 核心接入验证——一个固定的、不接业务逻辑的单标签终端。已确认 `iced_term 0.8.0` 锁定的 `iced` 版本（`0.14.0`）跟本项目一致。
- **Round 2（已完成，已自动化验证）**：多标签管理（开/关/切换标签）+ Profiles"打开终端"/Projects"接续"新增内嵌标签页入口（外部窗口路径保留，两者并存）。
- **验证方式变更**：用户明确要求不要每轮都被叫去手动验证，改成用 Win32 API 合成鼠标/键盘输入 + 截图自证，把 Round 1/2 一次性验证掉（真敲了命令、真看到回显、真开关切换了标签、真点了"打开终端（内嵌）"并看到内嵌终端里跑起了真实的 `claude` 首次运行向导），细节见 `06.13`。真人使用体验（好不好用）留到后面一次性验收，不再逐轮打断用户。
- **Round 3（已完成，已自动化验证）**：接续前"上次用的 Profile 是 X，是否沿用？"确认/更换 UI（`06.4` 第 3 步，之前是静默直接用记录里的 Profile）。用户明确表示不想再逐轮等待确认，这轮开始不再暂停等待"继续"指令，做完自动验证/提交/推送。细节见 `06.14`。**`06.4` 描述的完整体验至此已实现。**
