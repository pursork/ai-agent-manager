...
- **Round 1（已完成）**：`iced_term` 核心接入验证——一个固定的、不接业务逻辑的单标签终端，先把"真实 PTY 能不能嵌进 iced 窗口"这个核心技术风险验证掉。已确认 `iced_term 0.8.0` 锁定的 `iced` 版本（`0.14.0`）跟本项目一致；已用进程父子关系验证过真的拉起了一个以 `aam-gui.exe` 为父进程的 `powershell.exe`、关闭时干净退出、不留孤儿进程。键盘输入/渲染这部分自动化工具验证不了，交由用户亲自确认，细节见 `06.11`。
- **Round 2（已完成，Round 1 尚待用户实测确认）**：多标签管理（开/关/切换标签）+ Profiles"打开终端"/Projects"接续"新增内嵌标签页入口（外部窗口路径保留，两者并存）。这轮是在用户明确表示"Round 1 还没来得及试"、接受连带风险的情况下继续推进的，细节和验收方式见 `06.12`。
- **Round 3+（待立项）**：`06.4` 描述的"点击项目 → 自动开新标签页并 resume"完整体验里还没做的部分（如果 Round 2 用户实测后还有缺口）。
