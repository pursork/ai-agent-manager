...
///
/// Generic over `IntoFragment` (not just `&'a str`, Phase 5 Round 2)
/// since dynamically-built labels (e.g. a tab's `format!`-ed title) are
/// owned `String`s -- `IntoFragment` is implemented for both, so callers
/// don't need to route through an intermediate lifetime-tied borrow.
